...
$.fn.multipleSelect = function(options){
  var i18n = $.fn.multipleSelect.i18n;
  ...
  var buildSelector = function(elem, settings, wrapper){
    if ($(elem).is("select")) {
      ...
      // var itemWidth = Math.ceil((parseInt(width, 10)) / parseInt(columns,
		// 10)) - 11;
  ...
  var createTabs = function(){
    ...
    i18n.viewAll + '</li><li class="view_selected">' +
    i18n.selected+' (<strong class="view_selected_count">0</strong>)</li>' +
    '<li class="view_notselected">' + i18n.notSelected+ '</li></ul>' +
    '</div>');
  };

  var createSelectAllNone = function(){
    return $('<a class="selectall">' + i18n.selectAll + '</a> | <a class="selectnone">' + i18n.unselectAll + '</a>');
  };

  var createErrorMessage = function(wrapper) {
    $(".selector", wrapper).html($('<div class="error">' + i18n.couldNotRetrievieTheRemoteData + '</div>'));
    ...
    if (settings.content!==undefined) {
      ...
      $.ajax({
        ...
        beforeSend: function(request) {
          $(".selector", this.elem.selector).html($('<div class="loading">' + i18n.loading + '</div>'));
        },
        ...
        success: function(data, status, request) {
          if (data === null || data.size == 0) {
            ...
            if ($(this.elem).attr('data-disabled')) {
              var disabled_values = $(this.elem).attr('data-disabled').split(",");
            }
            $(data).each(function() {
              ...
              option.attr('disabled', (disabled_values.indexOf(this[settings.contentValue].toString()) != -1));
              option.data(this);
              $(elem).append(option);
            });
...
$.fn.multipleSelect.i18n = { 
		...
		couldNotRetrievieTheRemoteData : 'Could not retrieve the remote data.',
};
